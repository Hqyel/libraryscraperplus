...
class LibraryScraper(_PluginBase):
  ...
  # TMDB 完结状态集合
  _ENDED_STATUSES = {"Ended", "Canceled"}
  ...
  def init_plugin(self, config: dict = None):
    ...
    if config:
      ...
      self._follow_only = config.get("follow_only") or False
      self._clear_cache = config.get("clear_cache") or False

    # 处理清除缓存
    if self._clear_cache:
      logger.info("媒体库刮削：清除媒体完结状态缓存")
      self.save_data("media_status", {})
                ...
                'content': [
                  {
                    ...
                    'props': {
                      'type': 'info',
                      'variant': 'tonal',
                      'text': '【只处理追更】开启后，将自动跳过TMDB中标记为已完结(Ended)或已取消(Canceled)的剧集，'
                              '仅刮削连载中的剧集和电影。首次运行时会自动建立完结状态缓存，'
                              '后续运行将直接跳过已完结的剧集。如需重新全量扫描，可开启"清除完结状态缓存"。'
                    }
                  }
                ]
              }
  ...
  def get_page(self) -> List[dict]:
    ...
    """
    # 获取媒体状态数据
    media_status: dict = self.get_data("media_status") or {}
    if not media_status:
      return [
        {
          'component': 'div',
          'text': '暂无媒体完结状态数据，请先运行一次刮削。',
          'props': {
    ...
    ongoing_items = []
    ended_items = []
    for key, info in media_status.items():
      item = {
    ...
    # 构建表格行
    table_rows = []
    for item in ongoing_items + ended_items:
      table_rows.append({
        'component': 'tr',
        'content': [
          {
            ...
            'content': [
              {
                'component': 'VChip',
                'props': {
                  'color': 'error' if item["ended"] else 'success',
                  'size': 'small',
                },
                'text': '已完结' if item["ended"] else '连载中',
              }
            ]
          },
          {
    ...
    return [
      ...
      },
      {
        ...
        'text': f'共 {len(media_status)} 个媒体 | '
                f'连载中 {len(ongoing_items)} 个 | '
                f'已完结 {len(ended_items)} 个'
      }
    ]
  ...
  def __scrape_dir(self, path: Path, mtype: MediaType):
    ...
    # 追更模式：在识别媒体之前，先用本地nfo中的tmdbid查询缓存
    if self._follow_only and tmdbid:
      if self.__is_media_ended(tmdbid=str(tmdbid), mtype=mtype):
        logger.info(f"{path} 已完结（缓存命中），跳过刮削")
        return
    ...
    # 追更模式：识别后再次检查（针对没有nfo但能通过名称识别到tmdbid的情况）
    if self._follow_only and mediainfo.tmdb_id:
      if self.__is_media_ended(tmdbid=str(mediainfo.tmdb_id), mtype=mtype):
        logger.info(f"{path} ({mediainfo.title}) 已完结（缓存命中），跳过刮削")
        return
    ...
    # 追更模式：刮削完成后更新媒体完结状态
    if self._follow_only and mediainfo.tmdb_id:
      self.__update_media_status(mediainfo=mediainfo, mtype=mtype)

  def __is_media_ended(self, tmdbid: str, mtype: MediaType) -> bool:
    """
    检查媒体是否已完结（从本地缓存中查询）
    :param tmdbid: TMDB ID
    :param mtype: 媒体类型
    :return: True=已完结, False=未完结或未记录
    """
    media_status: dict = self.get_data("media_status") or {}
    status_key = f"tmdb_{tmdbid}_{mtype.value}"
    cached = media_status.get(status_key)
    if cached and cached.get("ended"):
      return True
    return False

  def __update_media_status(self, mediainfo, mtype: MediaType):
    """
    更新媒体完结状态到本地缓存
    ...
    """
    try:
      # 判断是否完结：电影一律视为完结，电视剧根据TMDB status判断
      if mtype == MediaType.MOVIE:
        is_ended = True
        tmdb_status = getattr(mediainfo, "status", None) or "Released"
      else:
        tmdb_status = getattr(mediainfo, "status", None) or ""
        is_ended = tmdb_status in self._ENDED_STATUSES

      media_status: dict = self.get_data("media_status") or {}
      status_key = f"tmdb_{mediainfo.tmdb_id}_{mtype.value}"
      media_status[status_key] = {
        "title": mediainfo.title or "未知",
        "ended": is_ended,
        "status": tmdb_status,
        "updated": datetime.now().strftime("%Y-%m-%d %H:%M")
      }
      self.save_data("media_status", media_status)
      logger.info(
        f"媒体状态已更新：{mediainfo.title} - "
        f"TMDB状态={tmdb_status}, 完结={is_ended}"
      )
    except Exception as e:
      logger.warn(f"更新媒体完结状态失败：{str(e)}")
